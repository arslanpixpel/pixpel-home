...
import useCCDWalletStore from "../common/ccdWalletStore";
...
export const useCCDWallet = () => {
    const provider = useAsyncMemo(detectConcordiumProvider, noOp, []);
    const hasWallet = provider !== undefined;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const hasNewApi = (provider as any)?.getSelectedChain !== undefined;
    const walletStore = useCCDWalletStore();
    const { deleteWallet, setWallet, account, isActive } = walletStore || {};
    ...
    const refreshMostRecentlySelectedAccount = useCallback(async () => {
        ...
        if (account) {
            {
                setWallet ? setWallet(account as string) : "";
            }
        }
    }, [setWallet, provider]);
