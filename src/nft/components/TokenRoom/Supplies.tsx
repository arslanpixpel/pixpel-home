...
const Supplies = ({ title, value }: Supplies) => {
        ...
        <>
            <div className="flex flex-col w-80 h-80 py-5 px-4 bg-app-black-button rounded-md">
                <div className="flex justify-between items-center">
                    <input type="checkbox" />
                    <div
                        className="bg-app-black-select py-2 px-3 gap-2text-[#717A8B]  text-sm rounded-md flex items-center cursor-pointer"
                        onClick={() => setShowModal(true)}
                    >
                        <Image src={help} className="w-4 h-4" alt="help" />
                        INFO
                    </div>
                </div>
