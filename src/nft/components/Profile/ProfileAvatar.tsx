...
const ProfileAvatar = () => {
    ...
    const { tempData, userData, type, userImg, setUserImg } = useAppContext();
    // console.log("myTypo : " , typo);
    const userDataDev = tempData?.player;
    const realData = userData?.player;
...
useEffect(() => {
    ...
    const handleFileUpload = async () => {
        ...
        await upload.then(async (err: any) => {
            ...
            await axios
                .put(`/players/updateImg/${realData?.id}`, {
    ...
    return <>
            <div className="flex max-sm:flex-col bg-app-black rounded-lg px-6 py-5 2xl:w-[1317px] w-full mt-[20px] 2xl:ml-4">
            <img
                src={
                    userImg
                ...
                alt="Profile"
                className="!w-[118px] !h-[118px] rounded-full mr-6"
            />
            <Image
            ...
            <div className="flex flex-col">
                <p className="font-medium leading-normal text-[18px] md:text-lg text-xl">
                    {userDataDev ? userDataDev?.name : realData?.name ? realData?.name : "PLAYER"}
                </p>
                ...
                <p className="font-normal text-[13px] leading-normal sm:text-base text-sm text-app-black-duration">
                    ...
                    <div className="flex max-xl1:flex-col gap-12 w-full">
                        {/* <div className="flex flex-col gap-2 text-[18px] font-normal w-1/2 max-xl1:w-full">
                            ...
                            <input
                                ...
                                placeholder={userDataDev ? userDataDev?.name : realData?.name ? realData?.name : "PLAYER" }
                                disabled
                            />
                        </div>
                        <div className="flex flex-col gap-2 text-[18px] font-normal w-1/2 max-xl1:w-full">
                            Email:
                            <input className="bg-app-black-button h-16 rounded-lg px-3" placeholder={userDataDev ? userDataDev.email : realData.email ? realData.email : "PLAYER@gmail.com"} disabled/>
                        </div> */}
                    </div>
                    <div className="flex max-xl1:flex-col gap-12">
                        {/* <div className="flex flex-col gap-2 text-[18px] font-normal w-1/2 max-xl1:w-full">
                            Phone Number:
                            <input className="bg-app-black-button h-16 rounded-lg px-3" placeholder={userDataDev ? userDataDev.contact_details : realData.contact_details ? realData.contact_details : "9909893843"} />
                        </div>
                        <div className="flex flex-col gap-2 text-[18px] font-normal w-1/2 max-xl1:w-full">
                            Address:
                            <input className="bg-app-black-button h-16 rounded-lg px-3" placeholder={userDataDev ? userDataDev.address : realData.address ? realData.address : "abc near abc street"} />
                        </div> */}
                    </div>
