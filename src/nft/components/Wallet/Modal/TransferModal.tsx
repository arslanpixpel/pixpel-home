...
import CoinDropDown from "../Transaction/CoinDropDown";
                    ...
                    <div className="fixed inset-0 z-10 overflow-y-auto">
                        ...
                        <div className="flex items-center px-4 py-6 min-h-screen">
                            <div className="relative flex flex-col w-full max-w-3xl p-4 mx-auto bg-app-black-modal rounded-xl shadow-lg xs:px-6 px-2 py-6 text-lg">
                                <div className="flex justify-center text-2xl my-10">internal Transferson PIXPEL</div>
                                <div className="flex my-2">
                                    <div className=" text-[#717A8B]  text-sm w-5/12">{accountList[0].title}</div>
                                    <div className="w-28"></div>
                                    <div className=" text-[#717A8B]  text-sm">{accountList[1].title}</div>
                                </div>
                                ...
                                <div className="flex gap-3 flex-col xs:flex-row">
                                    <div className="xs:w-1/2">
                                        {/* <Dropdown initialContent="COIN" contentList={coinList}></Dropdown> */}
                                        <CoinDropDown
                                            initialContent="Coin"
                                            contentList={coinList.map((item) => ({
                                                title: item.title,
                                                imageSrc: item.img,
                                            }))}
                                        />
                                    ...
                                    <div className="bg-app-black flex justify-between items-center xs:w-1/2 py-2 px-4 rounded-md">
                                        <div className=" text-[#717A8B] ">AMOUNT</div>
                                        <div className="text-blue-500">MAX</div>
                                    </div>
                                </div>
                                <div className=" text-[#717A8B]  flex justify-end my-2 text-sm">
                                    Available Amount: 50 000.000
                                </div>
                                <div
                                    className="flex justify-center items-center w-full rounded my-3 bg-[#2EBD85] hover:bg-[#1FF19F] py-3 cursor-pointer"
                                    onClick={() => props.setShowModal(false)}
                                >
                                    CONFIRM
                                </div>
