    ...
    return (
        <>
            <div className="flex flex-col">
                <div className="text-2xl mb-3 text-[30px] font-semibold">Deposit Fiat</div>
                {tempOverview}
                <div className="flex mt-3 gap-3 mb-9">
                    <TokenButton title="BANKS" selected={selected === 0} handleClick={handleClick(0)} />
                    <TokenButton title="CARDS" selected={selected === 1} handleClick={handleClick(1)} />
                </div>
                <div className="flex justify-between items-baseline">
                    <div className="text-[24px] leading-normal font-medium">{word}</div>
                    <div
                        className="bg-app-green hover:bg-[#1FF19F] px-5 py-4 rounded-md hover:cursor-pointer mx-8 p"
                        onClick={() => {
                            setShowModal(true);
                        }}
                    >
