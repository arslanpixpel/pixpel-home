...
import { Form, Tabs, Checkbox, Tooltip, Modal, Input, DatePicker, Select, Button } from "antd";
...
type Props = {
    ...
    setTabsActiveIndex: React.Dispatch<React.SetStateAction<number>>;
    setBackToSell: () => void;
    tabkey: string;
...
const Sell = ({
    ...
    const cardItems: TabsProps["items"] = [
        {
            ...
            children: (
                <Set
                    largeTabView={largeTab}
                    nftInfoBox={openInfoBox}
    ...
    return (
        <div className="flex flex-col w-full relative">
            {tabkey === "1" && (
                <div className="flex flex-col absolute left-0 right-0 -top-2 z-30 w-4/5 mx-auto">
                    <Tooltip
                        placement="right"
                        color="#37404C"
                        className="font-semibold"
                        title={"Choose the NFTs fot the SET"}
                    >
                        <button
                            className="inline-flex mx-auto px-8 py-4 text-lg font-semibold rounded-md bg-app-green mb-4 hover:bg-white hover:text-app-green"
                            onClick={handleOpen}
                        >
                            + Add
                        </button>
            ...
            <Tabs
                activeKey={tabkey}
                type="card"
                className="w-full shoppingTabs inventoryTabs"
                items={cardItems}
                onChange={onChange}
            />
            <div className={`${tabkey !== "0" ? "hidden" : "flex"}  bg-app-black py-3 gap-3 px-6 ${tabkey}`}>
                {openInfoBox ? (
                    <>
                                          ...
                                          : `${
                                                openInfoBox
                                                    ...
                                                    : `${
                                                          largeTab
                                                              ? "xl1:grid-cols-4 2xl1:grid-cols-5 3xl1:grid-cols-6"
                                                              : "xl1:grid-cols-5 2xl1:grid-cols-7 3xl1:grid-cols-8 4xl1:grid-cols-10"
                                                      }`
                                            }`
                                  }`
                        } gap-3`}
                    >
                        {new Array(49).fill(null).map((_, index) => {
                            return (
                                <div
                                    key={index}
                                    className="flex flex-col gap-y-[15px] relative bg-app-black-button p-4   max-h-[232px] rounded-md"
                                >
                                    <div className="flex bg-app-black-select p-4 hover:bg-app-black-duration">
                                        <Image src={NFTCrypto} className="w-full max-w-full" alt="" />
                                    </div>
