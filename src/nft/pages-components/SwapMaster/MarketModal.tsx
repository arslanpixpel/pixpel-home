import { marketSuccessHandle } from "@nft/actions/market";
import { useAppDispatch } from "@nft/hooks";

interface LiquidityModal {
    setShowModal: (val: boolean) => void;
    setShowSucessModal: (val: boolean) => void;
    showModal?: boolean;
}
export default function MarketModal(props: LiquidityModal) {
    const dispatch = useAppDispatch();
    const handleClick = () => {
        props.setShowModal(false);
        props.setShowSucessModal(true);
                        ...
                        <div className="flex items-center min-h-screen px-4 py-8">
                            <div className="relative flex flex-col w-full sm:p-[50px] xs:p-[40px] 1xs:p-[30px] 2xs:p-[20px] p-[10px] mx-auto text-lg bg-app-black-modal rounded-xl xs:w-auto">
                                <div className="flex mb-5">
                                    <div className="text-xl font-medium 2xs:text-2xl">Security Verification</div>
                                </div>
                                <div className="flex flex-col gap-1 px-3 py-2 rounded-md 2xs:py-5 2xs:px-7 bg-app-black mb-7">
                                    ...
                                    <div className="flex flex-row gap-3 1xs:justify-between">
                                        ...
                                        <div className="flex w-56 1xs:w-4/5">
                                            <div className="text-xs font-medium 2xs:text-base">Tron (TRC20)</div>
                                        </div>
                                    </div>
                                    <div className="flex flex-row gap-6 1xs:gap-0 1xs:justify-between">
                                        ...
                                        <div className="flex w-56 1xs:w-4/5">
                                            <div className="text-xs font-medium 2xs:text-base">Funding Wallet</div>
                                        </div>
                                    </div>
                                </div>
                                <div className="flex">
                                ...
                                <div className="flex flex-row items-center justify-between h-16 px-2 py-5 mb-1 rounded-md 2xs:px-7 bg-app-black">
                                    ...
                                    <div className="text-xs 2xs:text-base text-slate-400">Verification code sent</div>
                                </div>
                                ...
                                <div className="flex mb-3">
                                    <div className="text-base font-medium ">Email Verification Code</div>
                                </div>
                                <div className="flex flex-row items-center justify-between h-16 px-2 py-5 mb-1 rounded-md 2xs:px-7 bg-app-black">
                                ...
                                <div
                                    className="flex items-center justify-center h-16 rounded-md cursor-pointer bg-[#2EBD85] hover:bg-[#1FF19F]"
                                    onClick={handleClick}
                                >
                                    <div className="text-lg font-medium" onClick={handleClick}>
                                        Submit
                                    </div>
                                </div>
                            </div>
