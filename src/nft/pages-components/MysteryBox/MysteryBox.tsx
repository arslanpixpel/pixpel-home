...
import Exclaim from "@public/assets/images/exclaimnation.svg";
...
const MysteryBox = () => {
    ...
    return (
        <>
            <div className="px-32 pt-11 pb-80 flex flex-col">
                <div className="text-3xl font-bold">Mystery Box Logistics</div>
                <div className="flex gap-8 my-10">
                    <div className="flex flex-col gap-4 w-1/2">
                        <div>Price:</div>
                        <div className="flex items-center justify-center bg-app-black py-4 rounded-md">500 PIXP</div>
                    </div>
                    <div className="flex flex-col gap-4 w-1/2">
                        <div className="flex justify-between">
                            <div>Total probability rate:</div>
                            <Image src={Exclaim} alt="help" className="w-5" />
                        </div>
                        <Probability percent={100} height={56} className={""} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-5 gap-10 my-8">
                    {WrappList.map((menu, idx) => {
                        return <NFTWithProbability key={idx} img={menu.img} percent={menu.percent} height={32} />;
                    })}
